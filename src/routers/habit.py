...
def create_habit(
    user_id: int, habit: schemas.HabitCreate, db: Session = Depends(get_db_session)
):
...
def track_habit(
    habit_id: int, progress: schemas.ProgressCreate, db: Session = Depends(get_db_session)
):
...
def get_stats(user_id: int, db: Session = Depends(get_db_session)):
