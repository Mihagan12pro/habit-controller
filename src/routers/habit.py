...
from src.services.habit import create_new_habit, track_progress, get_user_stats
...
def create_habit(user_id: int, habit: schemas.HabitCreate, db: Session = Depends(get_db)):
...
def track_habit(habit_id: int, progress: schemas.ProgressCreate, db: Session = Depends(get_db)):
